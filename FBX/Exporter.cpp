...
void Fbx::Exporter::ReadBoneData(FbxNode* node, int index, int parent)
{
	...
	if (attribute != NULL) {
		...
		bool b = false;	//b|중 하나라도 성립시 true
		...
		if (b == true) {
			...
			if (nodeType == FbxNodeAttribute::eMesh) {
				converter->Triangulate(attribute, true, true);	//an attribute, wechseln true, wie Legacy(False ist neue Version)
				//

				ReadMeshData(node, index); //wenn Node kein gibt, return NULL.
			}
		}//if(b)
...
{
	FbxMesh* mesh = node->GetMesh();	//Get mesh from node.

	vector<FbxVertex*> vertices;

	for (int p = 0; p < mesh->GetPolygonCount(); p++) {	//p=polygon
		int vertexInPolygon = mesh->GetPolygonSize(p);
		assert(vertexInPolygon == 3);

		for (int vi = vertexInPolygon-1; vi >= 0; vi--) {	//vi=vertexIndex	polygon ist meistens 3. es wechselt LH
			//RH wechseln LH
			FbxVertex* vertex = new FbxVertex();

			int cpIndex = mesh->GetPolygonVertex(p, vi);	//cpIndex = controlPointIndex
			...
			D3DXVec3TransformCoord(&vertex->Vertex.Position, &temp, &Utility::Negative());	//Negative ; RH wechseln LH

			//normalVector
			FbxVector4 normal;
			mesh->GetPolygonVertexNormal(p, vi, normal);
			normal.Normalize();	//Direction muss immer normalizen.
			temp = Utility::ToVector3(normal);
			D3DXVec3TransformCoord(&vertex->Vertex.Normal, &temp, &Utility::Negative()); //Es wechseln auch Location.
			//Normal gibt kein Bewegung, deshalb coord, normal gleich ist.
...
/*
Mesh = 텍스쳐의 재질 전체
MeshPart = Mesh안에서 Material기준으로 나뉜 파트
*/
...
void Fbx::Exporter::ReadSkinData()
{
	...
	for (FbxMeshData* meshData : meshDatas) {
		for (int i = 0; i < scene->GetMaterialCount(); i++) {
			...
			if (gather.size() < 1) continue;	//Wenn es Keines gibt, continue.
...
{
//machen Mesh Data File.

	Path::CreateFolder(saveFolder);	//Wenn es kein Folder gibt, macht Folder.

	//macht File wie BinaryForm.
